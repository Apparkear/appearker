...
<div class="page-container">
    ...
    <div class="page-content-wrapper">
                    ...
                    <div class="portlet box blue">
                        ...
                        <div class="portlet-body">
                            ...
                            <table class="table table-striped table-hover table-bordered" id="sample_editable_1">
                                ...
                                <tbody>
                                    <?php
                                    $sql1 = mysql_query("select * from `estejmam_booking` where `user_id` = '" . $_REQUEST['id'] . "'");
                                    $num = mysql_num_rows($sql1);
                                    if ($num > 0) {

                                        $count = 1;
                                        while ($row = mysql_fetch_object($sql1)) {
                                            //$fetch_user = mysql_fetch_object(mysql_query("select * from `estejmam_user` where `id`='".$row->order_user_id."'"));
                                            // $fetch_store = mysql_fetch_object(mysql_query("select * from `estejmam_store` where `id`='".$row->storeid."'"));
                                            // $fetch_tblorder = mysql_fetch_object(mysql_query("select * from `estejmam_tblorders` where `orderid`='".$row->orderid."'"));
                                            // $fetch_billingdetails = mysql_fetch_object(mysql_query("select * from `estejmam_billing` where `billing_id`='".$fetch_tblorder->billingid."'"));
        ...
        <!-- <tr>
        <td>
        <a href="order_details.php?orderid=<?php echo $row->orderid; ?>"><?php echo $row->new_order_id; ?></a>
                </td>
                <td>
                ...
                <td>
                         <button class="btn btn-mini green" type="button"  onClick="location.href='order_details.php?orderid=<?php echo $row->orderid; ?>'">Details</button>&nbsp;
                                            ...
                                            <tr>
                                                ...
                                                <td><?php
                                    $estenjman_start_date = strtotime($row->start_date);
                                    echo date('Y-m-d', $estenjman_start_date);
                                    ?></td>
                                                <td><?php
                                    $estejman_end_date = strtotime($row->end_date);
                                    echo date('Y-m-d', $estejman_end_date);
                                    ?></td>
                                                <td><?php echo $fetch_property->name; ?></td>
                                                <td><?php echo $row->total_days; ?></td>
                                                <td><a href="view_booking_details.php?id=<?php echo $row->id; ?>" class="btn btn-mini green">Booking Details</a></td>
                                            </tr>
                                                    <?php
                                                }
                                            } else {
