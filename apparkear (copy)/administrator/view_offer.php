...
<!--									<div class="col-md-6">
							...
							<tbody>
							<?php
                                                        $fetch_product=mysql_query("select * from estejmam_product_bid  where `product_id`='".$_REQUEST['id']."'");
                                                        $num=mysql_num_rows($fetch_product);
                                                        if($num>0)
                                                        {
                                                        while($product=mysql_fetch_array($fetch_product))
                                                        {
                                                            $fetch_user=mysql_fetch_array(mysql_query("select * from `estejmam_user` where `id`='".$product['user_id']."'"));
                                                            $fetch_product1=mysql_fetch_array(mysql_query("select * from `estejmam_product` where `id`='".$product['product_id']."'"));
                                                            $image=mysql_fetch_array(mysql_query("SELECT * FROM `estejmam_moreimage` WHERE `pro_id`='".$fetch_product['id']."' limit 1"));
                                                            if($image['image']!='')
                                                            {
							...
							<tr>
								...
								<td>
									<?php echo stripslashes($fetch_user['fname']);?>
								</td>
                                                                <td>
									<?php echo stripslashes($fetch_product1['name']);?>
								</td>
                                                                <td>
                                                                    <?php echo stripslashes($product['offer_price']);?>
                                                                </td>
                                                                <!--<td>
                                                                    <a  href="details_product.php?id=<?php echo $product['id'] ?>&action=details">Details</a>
                                                                    <a  href="view_offer.php?id=<?php echo $product['id'] ?>&action=details">View Offer</a>
								</td>-->
								...
								<td>
                                                                    ...
                                                                    {
                                                                    ?>
									<button class="btn btn-mini green" type="button"  >Approve</button>&nbsp;
                                                                        <?php
                                                                    }
 else {
     ?>
                                                                        <button class="btn btn-mini red" type="button"  >Disapprove</button>&nbsp;
                                                                        <?php
 }
                                                                        ?>
								</td>
