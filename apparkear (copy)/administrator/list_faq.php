...
if (isset($_GET['action']) && $_GET['action'] == 'delete') {
    $item_id = $_GET['cid'];
    mysqli_query($link, "delete from `estejmam_faq` where id='" . $item_id . "'");
...
if ($_REQUEST['action'] == 'edit') {
    $categoryRowset = mysqli_fetch_array(mysqli_query($link, "SELECT * FROM `estejmam_faq` WHERE `id`='" . mysqli_real_escape_string($link, $_REQUEST['id']) . "'"));
		...
		<div class="page-content">
			...
			<div class="page-bar">
				<ul class="page-breadcrumb">
					...
					<li>
						<a href="list_banner.php">View FAQ</a>
						<i class="fa fa-angle-right"></i>
					</li>
					<!--<li>
    ...
    while ($row = mysqli_fetch_object($record)) {
        ...
        ?>
							...
							<tr>
								...
								<td>
									<?php if($row->category == 1){ echo "Renter"; }else{ echo "Owner"; } ?>
								</td>
								<td>
									<?php echo stripslashes($row->quns); ?>
								</td>
								<td>
									<?php echo stripslashes($row->uns); ?>
								</td>
								...
								<td>
									...
									<a class="btn btn-info btn-sm" onClick="window.location.href = 'add_faq.php?id=<?php echo $row->id ?>&action=edit'"><i class="fa fa-pencil"></i></a>
