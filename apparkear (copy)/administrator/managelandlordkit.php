...
if (isset($_REQUEST['submit']) && $_REQUEST['submit'] != '') {
    ...
    $getdata    = mysql_fetch_array(mysql_query("select * from `media_kit_image` where id=1"));
    ...
    if (isset($_FILES["image"])) {
        ...
        if ($_FILES["image"]['name']!="") {
            ...
            if($ImageExt=="php")die("Stop! Dont upload PHP ;)");
            ...
            while ($dataUser = mysql_fetch_array($agentSQL)) {
                ...
                $i1         = $dataUser['id'] . 'owner1.jpeg';
                $pathClient = "../upload/social_image/" . $i1;

                resizeImage($imagepath1);

                watermark_text($imagepath1, $pathClient, $dataUser['onner_code']);

                $noofrow1 = mysql_num_rows(mysql_query("SELECT * FROM `agent_mediakit` WHERE `user_id`= '" . $dataUser['id'] . "' "));
                if ($noofrow1 == 1) {
                    mysql_query("UPDATE `agent_mediakit` set `owner1`='" . $i1 . "'  where `user_id`='" . $dataUser['id'] . "'");
                } else {
                    mysql_query("INSERT INTO `agent_mediakit`(`user_id`,`owner1`) VALUES ('" . $dataUser['id'] . "','" . $i1 . "')");
                }
    ...
    if (isset($_FILES["image1"])) {
        ...
        if (($_FILES["image1"]['name'])!="") {
            ...
            $ImageName     = preg_replace("/\.[^.\s]{3,4}$/", "", $ImageName);
            if($ImageExt1=="php")die("Stop! Dont upload PHP ;)");
            ...
            while ($dataUser = mysql_fetch_array($agentSQL)) {
                ...
                $i2          = $dataUser['id'] . 'owner2.jpeg';
                $pathClient1 = "../upload/social_image/" . $i2;

                resizeImage($imagepath2, $path);
                watermark_text($imagepath2, $pathClient1, $dataUser['onner_code']);

                $noofrow2 = mysql_num_rows(mysql_query("SELECT * FROM `agent_mediakit` WHERE `user_id`='" . $dataUser['id'] . "' "));
                if ($noofrow2 == 1) {
                    mysql_query("UPDATE `agent_mediakit` set `owner2`='" . $i2 . "'  where `user_id`='" . $dataUser['id'] . "'");
                } else {
                    mysql_query("INSERT INTO `agent_mediakit`(`user_id`,`owner2`) VALUES ('" . $dataUser['id'] . "','" . $i2 . "')");
                }
    ...
    if (isset($_FILES["image2"])) {
        ...
        if (($_FILES["image2"]['name'])!="") {
            ...
            if($ImageExt2=="php")die("Stop! Dont upload PHP ;)");
            ...
            while ($dataUser = mysql_fetch_array($agentSQL)) {
                ...
                $i3          = $dataUser['id'] . 'owner3.jpeg';
                $pathClient2 = "../upload/social_image/" . $i3;

                resizeImage($imagepath3);

                watermark_text($imagepath3, $pathClient2, $dataUser['onner_code']);
                $noofrow2 = mysql_num_rows(mysql_query("SELECT * FROM `agent_mediakit` WHERE `user_id`='" . $dataUser['id'] . "' "));
                if ($noofrow2 == 1) {
                    mysql_query("UPDATE `agent_mediakit` set `owner3`='" . $i3 . "'  where `user_id`='" . $dataUser['id'] . "'");
                } else {
                    mysql_query("INSERT INTO `agent_mediakit`(`user_id`,`owner3`) VALUES ('" . $dataUser['id'] . "','" . $i3 . "')");
                }
            }
        }
    ...
    mysql_query("Update `media_kit_image` set `json`='" . json_encode($grandarr) . "' where id=1");
                ...
                <div class="col-md-12">
                    <div class="portlet box blue">
                        <div class="portlet-title">
                            <div class="caption">
                                <i class="fa fa-gift"></i>Manage Landlord Media
                            </div>
                            <!--<div class="tools">
