...
$bchk = mysql_query("select * from `estejmam_main_property` where `city` like '%" . $_REQUEST['val'] . "%' order by `id` ASC");
while ($allcity = mysql_fetch_array($bchk)) {
    $allids[] = $allcity['id'];
}
$allnewids = implode(',', array_unique($allids));


$query = "SELECT * FROM estejmam_booking where `prop_id` IN($allnewids) and `status`='1'";
