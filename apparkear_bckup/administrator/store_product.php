...
?>
<?php
$user_name = mysql_fetch_array(mysql_query("select * from `estejmam_user` where `id`='".$_REQUEST['id']."'"));
?>
<?php
if(isset($_REQUEST['bulk_delete_submit'])){
        ...
        foreach($idArr as $id){
            ...
            mysql_query("DELETE FROM  `estejmam_product` WHERE orderid=".$id);
        }
        $_SESSION['success_msg'] = 'Products have been deleted successfully.';
        
        //die();
        
        header("Location:store_product.php");
    }
?>
...
<script language="javascript">
   function del(aa,bb)
   {
      ...
      if (a)
      {
        location.href="store_product.php?cid="+ aa +"&action=delete"
      }  
   } 
   
function inactive(aa)
   { 
       location.href="store_product.php?cid="+ aa +"&action=inactive"

   } 
   function active(aa)
   {
     location.href="store_product.php?cid="+aa+"&action=active";
   } 
...
<div class="page-container">
	...
	<div class="page-content-wrapper">
		<div class="page-content">
			...
			<!-- BEGIN PAGE HEADER-->
			<h3 class="page-title">
			Property list of <?php echo $user_name['fname'].' '.$user_name['lname']; ?>
			</h3>
			<div class="page-bar">
...
<!--									<div class="col-md-6">
							...
							<tbody>
							<?php
                                                        $fetch_product=mysql_query("select * from `estejmam_main_property`  where `user_id`='".$_REQUEST['id']."'");
                                                        ...
                                                        while($product = mysql_fetch_array($fetch_product))
                                                        {
                                                        	...
                                                        	//echo"SELECT * FROM `estejmam_image` WHERE `pro_id`='".$product['id']."' LIMIT 0,1";
                                                            $image = mysql_fetch_array(mysql_query("SELECT * FROM `estejmam_image` WHERE `prop_id`='".$product['id']."' LIMIT 0,1"));
