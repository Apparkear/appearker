...
if (isset($_REQUEST['submit'])) {
    ...
    $fields = array(
        'name' => mysqli_real_escape_string($link, $fname),
        'link' => mysqli_real_escape_string($link, $lname),
    ...
    if ($_REQUEST['action'] == 'edit') {
        ...
        if (in_array($fileParts['extension'], $fileTypes)) {
            $editQuery = "UPDATE `estejmam_partners` SET " . implode(', ', $fieldsList)
            ...
            if ($_FILES['image']['tmp_name'] != '') {
                ...
                $image = mysqli_query($link, "UPDATE `estejmam_partners` SET `logo`='" . $img_name . "' WHERE `id` = '" . mysqli_real_escape_string($link, $_REQUEST['id']) . "'");
            }
            ...
            $_SESSION['MSG'] = 1;
            header('Location:add_partners.php?id=' . $_REQUEST['id'] . '&action=edit');
            exit();
        }
    } else {
        ...
        if (in_array($fileParts['extension'], $fileTypes)) {
            ...
            $addQuery = "INSERT INTO `estejmam_partners` (`" . implode('`,`', array_keys($fields)) . "`)"
            ...
            if ($_FILES['image']['tmp_name'] != '') {
                ...
                $image = mysqli_query($link, "UPDATE `estejmam_partners` SET `logo`='" . $img_name . "' WHERE `id` = '" . $last_id . "'");
            }
...
if ($_REQUEST['action'] == 'edit') {
    $categoryRowset = mysqli_fetch_array(mysqli_query($link, "SELECT * FROM `estejmam_partners` WHERE `id`='" . mysqli_real_escape_string($link, $_REQUEST['id']) . "'"));
}
?>
