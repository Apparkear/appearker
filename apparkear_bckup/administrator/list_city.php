...
?>
<?php
$sql = "select * from `cities` where `state_id`='" . $_REQUEST['id'] . "' order by `name` ASC";
...
if (isset($_GET['action']) && $_GET['action'] == 'delete') {
    $item_id = $_GET['cid'];
    mysql_query("delete from estejmam_cities where id='" . $item_id . "'");
...
if (isset($_GET['action']) && $_GET['action'] == 'inactive') {
    $item_id = $_GET['cid'];
    mysql_query("update estejmam_cities set status='0' where id='" . $item_id . "'");
...
if (isset($_GET['action']) && $_GET['action'] == 'active') {
    $item_id = $_GET['cid'];
    mysql_query("update estejmam_cities set status='1' where id='" . $item_id . "'");
    //$_SESSION['msg']=message('updated successfully',1);
    header('Location:list_city.php');
    exit();
}
...
if (isset($_GET['action']) && $_GET['action'] == 'feature') {
    $item_id = $_GET['cid'];
    mysql_query("update `cities` set `is_featured`='1' where id='" . $item_id . "'");
    //$_SESSION['msg']=message('updated successfully',1);
    header('Location:list_city.php?id=' . $_REQUEST['sid']);
    exit();
}
if (isset($_GET['action']) && $_GET['action'] == 'unfeature') {
    $item_id = $_GET['cid'];
    mysql_query("update `cities` set is_featured='0' where id='" . $item_id . "'");
    //$_SESSION['msg']=message('updated successfully',1);
    header('Location:list_city.php?id=' . $_REQUEST['sid']);
    exit();
}
...
<script language="javascript">
    function del(aa, bb)
    {
        ...
        if (a)
        {
            location.href = "list_city.php?cid=" + aa + "&action=delete"
        }
    }

    function inactive(aa)
    {
        location.href = "list_city.php?cid=" + aa + "&action=inactive"

    }
    function active(aa)
    {
        location.href = "list_city.php?cid=" + aa + "&action=active";
    }
    function unfeature(aa, bb)
    {
        location.href = "list_city.php?cid=" + aa + "&action=unfeature&sid=" + bb;
    }
    function feature(aa, bb)
    {
        location.href = "list_city.php?cid=" + aa + "&action=feature&sid=" + bb;
    }
</script>
                ...
                <div class="col-md-12">
                    ...
                    <div class="portlet box blue">
                        ...
                        <div class="portlet-body">
                            ...
                            <input type="hidden" name="state_id" value="<?php echo $_REQUEST['id'] ?>">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php if ($row->is_featured == '0') { ?>
                                                        <a class="btn btn-success"  onClick="javascript:feature('<?php echo $row->id; ?>', '<?php echo $_REQUEST['id'] ?>');">Make Featured</a>
                                                    <?php } else { ?>
                                                        <a class="btn btn-danger"  onClick="javascript:unfeature('<?php echo $row->id; ?>', '<?php echo $_REQUEST['id'] ?>');">Unfeatured</a>
                                                    <?php } ?>
                                                </td>

                                                                                        <!--                                                <td>
                                                                                                                                            <a class="btn btn-info btn-sm" onClick="window.location.href = 'add_city.php?id=<?php echo $row->id ?>&action=edit'"><i class="fa fa-pencil"></i></a>
