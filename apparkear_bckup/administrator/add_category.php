...
{
	$item_id=$_GET['cid'];
	mysql_query("delete from  estejmam_amenities where id='".$item_id."'");
...
{
	 $item_id=$_GET['cid'];
	 mysql_query("update estejmam_amenities set status='0' where id='".$item_id."'");
...
{
	$item_id=$_GET['cid'];
	mysql_query("update estejmam_amenities set status='1' where id='".$item_id."'");
	$_SESSION['msg']=message('updated successfully',1);
	header('Location:list_category.php');
	exit();
}
	...
	$fields = array(
	 ...
	 if($_REQUEST['action']=='edit')
	  ...
	  $editQuery = "UPDATE `estejmam_amenities` SET " . implode(', ', $fieldsList)
		...
		$image =mysql_query("UPDATE `estejmam_amenities` SET `image`='".$img_name."' WHERE `id` = '" . mysql_real_escape_string($_REQUEST['id']) . "'");
		}
	 ...
	 $addQuery = "INSERT INTO `estejmam_amenities` (`" . implode('`,`', array_keys($fields)) . "`)"
		...
		$image =mysql_query("UPDATE `estejmam_amenities` SET `image`='".$img_name."' WHERE `id` = '" . $last_id . "'");
		}
...
if(isset($_REQUEST['bulk_delete_submit'])){
        ...
        foreach($idArr as $id){
            ...
            mysql_query("DELETE FROM `estejmam_amenities` WHERE id=".$id);
        }
				...
				<div class="col-md-12">
					<div class="portlet box blue">
									...
									<div class="portlet-body form">
										<!-- BEGIN FORM-->
										<form action="#" class="form-horizontal" method="post" action="add_category.php" enctype="multipart/form-data">
