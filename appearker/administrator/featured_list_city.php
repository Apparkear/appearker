...
?>
<?php
$sql = "select * from `cities` where `is_featured`='1'";
...
if (isset($_GET['action']) && $_GET['action'] == 'delete') {
    ...
    header('Location:featured_list_city.php');
    exit();
}
...
<script language="javascript">
    function del(aa, bb)
    {
        ...
        if (a)
        {
            location.href = "featured_list_city.php?cid=" + aa + "&action=delete"
        }
    }

    function inactive(aa)
    {
        location.href = "featured_list_city.php?cid=" + aa + "&action=inactive"

    }
    function active(aa)
    {
        location.href = "featured_list_city.php?cid=" + aa + "&action=active";
    }
    function unfeature(aa)
    {
        location.href = "featured_list_city.php?cid=" + aa + "&action=unfeature"

    }
    function feature(aa)
    {
        location.href = "featured_list_city.php?cid=" + aa + "&action=feature";
    }
</script>
                ...
                <div class="col-md-12">
                    ...
                    <div class="portlet box blue">
                        <div class="portlet-title">
                            <div class="caption">
                                <i class="fa fa-edit"></i>Featured City List
                            </div>
                            <div class="tools">
...
//$record=mysql_query($sql);
                                    if (mysql_num_rows($record) == 0) {
                                        ...
                                        while ($row = mysql_fetch_object($record)) {
                                            ...
                                            if ($row->image != '') {
                                                $image = "../upload/city_image/" . $row->image;
                                            } else {
                                                $image = "../upload/no.png";
                                            }
                                            ?>
                                            ...
                                            ?>"/></td>-->
                                                ...
                                                <td>
                                                    <?php if ($row->is_featured == '0') { ?>
                                                        <a class="btn btn-success"  onClick="javascript:feature('<?php echo $row->id; ?>');">Make Featured</a>
                                                    <?php } else { ?>
                                                        <a class="btn btn-danger"  onClick="javascript:unfeature('<?php echo $row->id; ?>');">Unfeatured</a>
                                                    <?php } ?>
                                                </td>


                                                <td>
                                                    <a class="btn btn-info btn-sm" onClick="window.location.href = 'add_city_image.php?id=<?php echo $row->id ?>&action=edit'">Add Image</a>
                                                </td>
...
<?php if (mysql_num_rows($record) > 0) { ?>
