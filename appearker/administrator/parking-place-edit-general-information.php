...
if (isset($_POST['submit'])) {
    $post_car_type_size = implode(',', $_POST['car_type_size']);

    $updateQry = mysqli_query($link, "UPDATE `parking_places` SET `car_type_size`='$post_car_type_size', `country`='{$_POST['country']}', `province` = '{$_POST['province']}', `city` = '{$_POST['city']}' WHERE `id` = '{$_REQUEST['id']}'");

    header('location: parking-place-edit-contact-information.php?id='.$_REQUEST['id']);
    exit();
}
                            ...
                            <form class="form-horizontal" method="post" action="" enctype="multipart/form-data">
                                <div class="form-body">
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-4">
                                            <select name="car_type_size[]" class="form-control" multiple required>
                                                <?php
                                                $type_size = explode(',', $editObj->car_type_size);
                                                while($car_type_size = mysqli_fetch_object($car_type_size_qry)){
                                                    echo '<option value="'.$car_type_size->id.'" '.(in_array($car_type_size->id, $type_size) ? 'selected' : '').'>'.$car_type_size->name.'</option>';
                                                }
                                                ?>
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-md-4">
                                            <select name="country" class="form-control" required>
                                                ...
                                                while($country = mysqli_fetch_object($countriesQry)){
                                                    echo '<option value="'.$country->id.'" '.(($editObj->country == $country->id) ? 'selected' : '').'>'.$country->name.'</option>';
                                                }
                                                ?>
                                            </select>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-4">
                                            <input type="text" name="province" class="form-control" value="<?php echo $editObj->province; ?>" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-4">
                                            <input type="text" name="city" class="form-control" value="<?php echo $editObj->city; ?>" required>
                                        </div>
                                    </div>
