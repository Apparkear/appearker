...
if (isset($_REQUEST['submit']) && $_REQUEST['submit'] != '') {
    $getdata    = mysql_fetch_array(mysql_query("select * from `media_kit_image` where id=2"));
    ...
    if (isset($_FILES["image"])) {
        ...
        if (($_FILES["image"]['name']) != "") {
            ...
            while ($dataUser = mysql_fetch_array($agentSQL)) {
                ...
                if ($noofrow1 == 1) {
                    mysql_query("UPDATE `agent_mediakit` set `client1`='" . $i1 . "'  where `user_id`='" . $dataUser['id'] . "'");
                } else {
                    mysql_query("INSERT INTO `agent_mediakit`(`user_id`,`client1`) VALUES ('" . $dataUser['id'] . "','" . $i1 . "')");
                }
            }
            $image = $NewImageName;
        }
    ...
    if (isset($_FILES["image1"])) {
        ...
        if (($_FILES["image1"]['name']) != "") {
            ...
            while ($dataUser = mysql_fetch_array($agentSQL)) {
                ...
                $i2          = $dataUser['id'] . 'client2.jpeg';
                $pathClient1 = "../upload/social_image/" . $i2;
                resizeImage($imagepath2);
                watermark_text($imagepath2, $pathClient1, $dataUser['client_code']);
                $noofrow2 = mysql_num_rows(mysql_query("SELECT * FROM `agent_mediakit` WHERE `user_id`='" . $dataUser['id'] . "' "));
                if ($noofrow2 == 1) {
                    mysql_query("UPDATE `agent_mediakit` set `client2`='" . $i2 . "'  where `user_id`='" . $dataUser['id'] . "'");
                } else {
                    mysql_query("INSERT INTO `agent_mediakit`(`user_id`,`client2`) VALUES ('" . $dataUser['id'] . "','" . $i2 . "')");
                }
    ...
    if (isset($_FILES["image2"])) {
        ...
        if (($_FILES["image2"]['name']) != "") {
            ...
            while ($dataUser = mysql_fetch_array($agentSQL)) {
                ...
                $i3          = $dataUser['id'] . 'client3.jpeg';
                $pathClient2 = "../upload/social_image/" . $i3;
                resizeImage($imagepath3);
                watermark_text($imagepath3, $pathClient2, $dataUser['client_code']);
                $noofrow2 = mysql_num_rows(mysql_query("SELECT * FROM `agent_mediakit` WHERE `user_id`='" . $dataUser['id'] . "' "));
                if ($noofrow2 == 1) {
                    mysql_query("UPDATE `agent_mediakit` set `client3`='" . $i3 . "'  where `user_id`='" . $dataUser['id'] . "'");
                } else {
                    mysql_query("INSERT INTO `agent_mediakit`(`user_id`,`client3`) VALUES ('" . $dataUser['id'] . "','" . $i3 . "')");
                }
            }
    ...
    mysql_query("Update `media_kit_image` set `json`='" . json_encode($grandarr) . "' where id=2");
                ...
                <div class="col-md-12">
                    <div class="portlet box blue">
                        <div class="portlet-title">
                            <div class="caption">
                                <i class="fa fa-gift"></i>Manage Tenand Media
                            </div>
                            <!--<div class="tools">
